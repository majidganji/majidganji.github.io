    ...
    var $ = function (elm) {
        ...
        Stars = function () {
            var num =
                    ...
                    750, //Math.abs(window.innerWidth - window.innerHeight),
                makeStar = function () {
                    return {
                        ...
                        speed: Math.random() * (c.width / c.height / 10),
                    };
                },
                stars = [],
                draw = function (star) {
                    ...
                    for (var i = 0; i < stars.length; i++) {
                        if (
                            star != stars[i] &&
                            Math.sqrt(
                                (star.pos.x - stars[i].pos.x) *
                                    (star.pos.x - stars[i].pos.x) +
                                    (star.pos.y - stars[i].pos.y) *
                                        (star.pos.y - stars[i].pos.y)
                            ) < 50
                        ) {
                            ctx.beginPath();
